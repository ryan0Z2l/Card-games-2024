...
/*TODO:*/
// Define the function return true if two images have overlapping otherwise return false
bool
Monster::get_touch(int x, int y, int w, int h){
    int offset = 0;
    // calculate the number of pictures before current direction
    for(int i=0; i<direction; i++)
        offset += direction_count[i];
    int mon_w = al_get_bitmap_width(moveImg[offset + sprite_pos]);
    int mon_h = al_get_bitmap_height(moveImg[offset + sprite_pos]);
    int mon_x = circle->x - mon_w/2;
    int mon_y = circle->y - (mon_h - grid_height/2);
    bool touch_x = false, touch_y = false;
    if( mon_x < x + w && mon_x + mon_w > x ) touch_x = true;
    if( mon_y < y + h && mon_y + mon_h > y ) touch_y = true;
